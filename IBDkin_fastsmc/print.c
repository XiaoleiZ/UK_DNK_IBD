...
void print_help(void)
{
    ...
    \t--map [file]\n\t\t#<string> genetic map with cM distance in plink format\n \
    \t--ind [file]\n\t\t#<string> a list individuals to be analyzed\n \
    \t--range [file]\n\t\t#<string> ranges of markers for each chromosome\n\n \
    (Optional parameters:)\n \
    \t--out ./\n\t\t#<string> output prefix\n \
    \t--nthreads 2\n\t\t#<int> number of threads\n \
    \t--kinship 0.0\n\t\t#<int> minimum kinship coefficient\n \
    \t--binkb 1000.0\n\t\t#<float> bin size in kbp to calculate IBD coverage\n \
    \t--fold 4.0\n\t\t#<float> max fold deviation to determine masked regions\n \
    \t--part 1 1\n\t\t#<int> <int> total partitions and current partition\n \
    \t--cutcm 4.0 2.0\n\t\t#<float> <float> minimum long and short IBD segment lengths in cM \n \
    \t--merge 5.0 20.0\n\t\t#<float> <float> max cM merge lengths for IBD1 and IBD2 regions \n\n \
    (Other flags:)\n \
    \t--outmask\n\t\t#output genome mask\n \
    \t--outcoverage\n\t\t#output IBD coverage\n \
    \n");
...
void p_parameters(void)
{
	...
	gzbuffer(fp, BUFF3);
	gzprintf(fp, "ID1\tID2\tsegnum\tibd1\tibd2\ttotg\n");
	gzclose(fp);
    }
    ...
    fprintf(stderr, " *ibd prob = %f*\n", IBDprob);
    fprintf(stderr, " *kinship = %f*\n", kincut);
...
void p_coverage(void)
{
	...
	if( minPos[chr] < maxPos[chr]){
		...
		if(g2 >= minPos[chr] && g1 <= maxPos[chr]){
		    ...
		    sprintf(str, "%s %.0f %.0f %f\n", chrs[chr], p1, p2, coverage[chr][i]);
		    gzputs(fp, str);
		}
	    }
	}
